      ...
      >
        <Toolbar sx={{ minHeight: { xs: 64, sm: 70 }, px: { sx: 2, md: 4 } }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexGrow: 1 }}>
            <SchoolIcon sx={{ color: 'secondary.main', fontSize: 32 }} />
            ...
            <Chip
              ...
              color="secondary"
              variant="soft"
              sx={{ fontWeight: 700, borderRadius: 1.5, bgcolor: '#fef2f2', color: '#db2777' }}
            />
          </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Tooltip title="Open Chat">
              <IconButton 
                ...
                sx={{ 
                  bgcolor: isChatOpen ? 'secondary.soft' : 'transparent',
                  color: isChatOpen ? 'secondary.main' : 'text.secondary',
                  transition: 'all 0.2s'
                }}
              >
                <ChatIcon />
            ...
            {menuItems.map((item, index) => (
              <ListItem key={item.text} disablePadding sx={{ mb: 0.5 }}>
                <ListItemButton
                  ...
                  to={item.path}
                >
                  <ListItemIcon sx={{ minWidth: 42, color: selectedIndex === index ? 'secondary.main' : 'text.secondary' }}>
                    {item.icon}
                  </ListItemIcon>
                  <ListItemText
                    primary={item.text}
